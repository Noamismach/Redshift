...
fn draw_info_panel(frame: &mut Frame, area: Rect, state: &UiState) {
    ...
    let sample_count = state.observations.lock().map(|obs| obs.len()).unwrap_or(0);
    let target_ip = state
        .target_ip
        .as_deref()
...
fn draw_chart(frame: &mut Frame, area: Rect, state: &UiState) {
    let observations = state
        .observations
        .lock()
        .map(|obs| collect_recent(obs.as_slice()))
        .unwrap_or_default();
    let skew_report = state.latest_report.lock().ok().and_then(|r| r.clone());

    let sample_points = build_sample_points(&observations, skew_report.as_ref());
    let regression_points = skew_report
        .as_ref()
...
fn build_sample_points(
    ...
    let freq = report
        ...
        .unwrap_or(1_000.0)
        .max(1.0);
    let base_time = observations
        .first()
        .map(|obs| obs.local_time)
        .unwrap_or(0.0);
...
fn compute_bounds(samples: &[(f64, f64)], regression: &[(f64, f64)]) -> ([f64; 2], [f64; 2]) {
    let mut max_x = samples.last().map(|(x, _)| *x).unwrap_or(1.0).max(1.0);
